...
Route::get('images/products/{filename}', [ProductImageController::class, 'show'])
    ->where('filename', '.*')
    ->name('product-images.show');
Route::get('images/avatar/{filename}', [MediaFileController::class, 'avatar'])->where('filename', '.*');
Route::get('images/brands/{filename}', [MediaFileController::class, 'brand'])->where('filename', '.*');
Route::get('images/banners/{filename}', [MediaFileController::class, 'banner'])->where('filename', '.*');
Route::get('images/store/{filename}', [MediaFileController::class, 'store'])->where('filename', '.*');
Route::get('images/leaves/{filename}', [MediaFileController::class, 'leave'])->where('filename', '.*');
Route::get('flags/{filename}', [MediaFileController::class, 'flag'])->where('filename', '.*');
Route::get('images/{filename}', [MediaFileController::class, 'app'])->where('filename', '.*');
...
if ($installed === true) {
    ...
    Route::middleware(['web', 'request.safety', 'store.enabled'])->group(function () {
        ...
        Route::prefix('online_store')->group(function () {
            ...
            Route::post('/contact/send', [MessageController::class, 'store'])->name('store.message.send');
