...
@php
  ...
  $assetPath = function ($p) {
      ...
      if (Str::startsWith($p, 'images/store/')) return store_asset_url($p);
      if (Str::startsWith($p, 'images/avatar/')) return avatar_image_url($p);
      if (Str::startsWith($p, 'images/brands/')) return brand_image_url($p);
      if (Str::startsWith($p, 'images/banners/')) return banner_image_url($p);
      if (Str::startsWith($p, 'flags/')) return flag_image_url($p);
      if (Str::startsWith($p, 'images/')) return app_image_url($p, asset($p));
      return asset($p);
  };
  ...
  $avatarSrc   = $avatar ? $assetPath($avatar) : null;
...
<body>
          ...
          @if($client)
            ...
            <div class="dropdown">
              <a class="btn btn-light d-flex align-items-center gap-2 dropdown-toggle" href="#" id="accountMenu"
                ...
                @if($avatarSrc)
                  <img src="{{ $avatarSrc }}"
                       onerror="this.src='{{ avatar_image_url(null) }}'"
  ...
  const NOIMG = "{{ product_image_url('no-image.png') }}";
