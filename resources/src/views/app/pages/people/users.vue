      ...
      >
        ...
        <template slot="table-row" slot-scope="props">
          <span v-if="props.column.field == 'actions'">
            ...
            <a
              @click="Lock_User(props.row)"
              v-if="currentUserPermissions && currentUserPermissions.includes('users_edit') && props.row.statut == 1"
              title="Lock Account"
              class="cursor-pointer ml-2"
              v-b-tooltip.hover
            >
              <i class="i-Unlock text-25 text-info"></i>
            </a>
            <a
              @click="Unlock_User(props.row)"
              v-if="currentUserPermissions && currentUserPermissions.includes('users_edit') && props.row.statut == 0"
              title="Unlock Account"
              class="cursor-pointer ml-2"
              v-b-tooltip.hover
            >
              <i class="i-Lock text-25 text-danger"></i>
            </a>
            <a
              @click="Reset_Edit_Limit(props.row)"
              v-if="currentUserPermissions && currentUserPermissions.includes('users_edit')"
              title="Reset Edit Limit"
              class="cursor-pointer ml-2"
              v-b-tooltip.hover
            >
              <i class="i-Repeat text-25 text-warning"></i>
            </a>
            <a
  ...
  methods: {
    ...
    Lock_User(user) {
      this.$swal({
        ...
        if (res.value) {
          this.Spin = this.$loading.show();
          axios.post(`users/${user.id}/lock`, {}).then(response => {
              this.Spin.hide();
              this.makeToast("success", response.data.message || "Account locked successfully.", "Success");
              this.Get_Users(this.serverParams.page);
          })
          .catch(error => {
              this.Spin.hide();
              this.makeToast("danger", "Failed to lock account.", "Error");
          });
        }
      })
    },
    ...
    Reset_Edit_Limit(user) {
      this.$swal({
        ...
        if (res.value) {
          this.Spin = this.$loading.show();
          axios.post(`users/${user.id}/reset-edit-limit`, {}).then(response => {
              this.Spin.hide();
              this.makeToast("success", response.data.message || "Edit limit reset successfully.", "Success");
              this.Get_Users(this.serverParams.page);
          })
          .catch(error => {
              this.Spin.hide();
              this.makeToast("danger", "Failed to reset edit limit.", "Error");
          });
        }
      })
    },
    ...
    Unlock_User(user) {
      this.$swal({
        ...
        if (res.value) {
          this.Spin = this.$loading.show();
          axios.post(`users/${user.id}/unlock`, {}).then(response => {
              this.Spin.hide();
              this.makeToast("success", response.data.message || "Account unlocked successfully.", "Success");
              // Refresh the user list to update the lock icon
              this.Get_Users(this.serverParams.page);
          })
          .catch(error => {
              this.Spin.hide();
              this.makeToast("danger", "Failed to unlock account.", "Error");
          });
        }
      })
    },
