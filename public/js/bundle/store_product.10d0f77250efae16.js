...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = ({
  ...
  methods: {
    ...
    //------ Generate code
    generateProductCodeValue: function generateProductCodeValue() {
      return String(Math.floor(Math.pow(10, 7) + Math.random() * (Math.pow(10, 8) - Math.pow(10, 7) - 1)));
    },
    generateNumber: function generateNumber() {
      this.code_exist = "";
      this.product.code = this.generateProductCodeValue();
    },
    applyDuplicateDefaults: function applyDuplicateDefaults() {
      var duplicatedCode = this.generateProductCodeValue();
      this.code_exist = "";
      this.product.code = duplicatedCode;
      if (this.variants.length) {
        this.variants = this.variants.map(function (variant, index) {
          return _objectSpread(_objectSpread({}, variant), {}, {
            code: "".concat(duplicatedCode, "-").concat(index + 1)
          });
        });
      }
    },
    ...
    Create_Product: function Create_Product() {
      ...
      axios.post("products", self.data).then(function (response) {
        ...
        nprogress__WEBPACK_IMPORTED_MODULE_1___default().done();
        self.SubmitProcessing = false;
        var errors = error && error.errors || error && error.response && error.response.data && error.response.data.errors || {};
        var errorGroups = Object.values(errors);
        var firstError = errorGroups.length > 0 && Array.isArray(errorGroups[0]) ? errorGroups[0][0] : null;
        var fallbackMessage = typeof error === "string" ? error : error && error.message || _this13.$t("InvalidData");
        if (errors.code && errors.code.length > 0) {
          self.code_exist = errors.code[0];
          _this13.makeToast("danger", errors.code[0], _this13.$t("Failed"));
        } else if (errors.variants && errors.variants.length > 0) {
          _this13.makeToast("danger", errors.variants[0], _this13.$t("Failed"));
        } else if (firstError) {
          _this13.makeToast("danger", firstError, _this13.$t("Failed"));
        } else {
          _this13.makeToast("danger", fallbackMessage, _this13.$t("Failed"));
        }
      });
    },
  ...
  created: function created() {
    ...
    if (duplicateId) {
      ...
      axios.get("products/".concat(duplicateId, "/edit")).then(function (response) {
        ...
        _this14.applyDuplicateDefaults();
      })["catch"](function () {
