...
class SyncService
{
    ...
    private function downloadWooProductImage(string $url, int $wooId): ?string
    {
        try {
            ...
            $filePath = product_images_prefix().'/'.$filename;
            $saved = Storage::disk(product_images_disk())->put($filePath, $body, [
                'visibility' => 'public',
                'ContentType' => $ct !== '' ? $ct : 'image/'.$ext,
            ]);
    ...
    private function productMainImageSrc(Product $product): ?string
    {
        try {
            ...
            if (! product_image_exists($imageName)) {
                return null;
            }
    ...
    private function resolveOrUploadWpMedia(Product $product, ?callable $shouldCancel = null): ?array
    {
        try {
            ...
            $key = product_image_storage_key($imageName);
            $stream = null;
            $size = null;
            $mime = 'image/jpeg';
            ...
            if (! is_resource($stream)) {
                ...
                if (! is_file($abs)) {
                    $abs = storage_path('app/public/images/products/'.$imageName);
                }
                if (! is_file($abs)) {
                    return null;
                }
    ...
    private function resolveOrUploadBrandImage(PosBrand $brand): ?array
    {
        try {
            ...
            try {
                ...
                $key = media_storage_key('brand', $imageName);
                if ($key && $disk->exists($key)) {
                    $stream = $disk->readStream($key);
                    $size = $disk->size($key);
                    $mime = $disk->mimeType($key) ?: $mime;
    ...
    private function downloadBrandImage(string $url, int $wooId): ?string
    {
        try {
            ...
            $mime = is_string($contentType) && trim($contentType) !== '' ? trim($contentType) : 'image/'.$ext;
            if (media_put('brand', $filename, $body, $mime)) {
                return $filename;
            }
        } catch (\Throwable $e) {
                ...
                try {
                    ...
                    if ($media && isset($media['id'])) {
                        ...
                        if ($imageFile !== '' && $imageFile !== 'no-image.png') {
                            $payload['image'] = ['src' => brand_image_url($imageFile)];
                        }
                    }
