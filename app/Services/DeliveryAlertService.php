...
class DeliveryAlertService
{
    ...
    protected function createSellerStatusAlert(
        ...
        $payload = [
            ...
            'delivery_name' => $actorRole === 'delivery' ? $deliveryName : '',
            'delivery_phone' => $actorRole === 'delivery' ? ($deliveryUser->phone ?? '') : '',
            'recorder_name' => $actorRole === 'record' ? $deliveryName : '',
            'recorder_phone' => $actorRole === 'record' ? ($deliveryUser->phone ?? '') : '',
            'event' => $type,
            'updated_at' => now()->toIso8601String(),
        ];
