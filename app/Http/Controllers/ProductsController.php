...
class ProductsController extends BaseController
{
    ...
    public function index(Request $request)
    {
        ...
        foreach ($products as $product) {
            ...
            $item['image_url'] = product_image_url($item['image']);
    ...
    {
        return product_images_disk();
    }

    private function productImagesPrefix(): string
    {
        return product_images_prefix();
    }

    private function productImageObjectKey(string $filename): string
    {
        return $this->productImagesPrefix().'/'.basename($filename);
    }

    private function ensureProductImagesDiskAvailable(): void
    {
        $disk = config('filesystems.disks.'.$this->productImagesDisk());
        if (! is_array($disk) || empty($disk['driver'])) {
            throw ValidationException::withMessages([
                'image' => ['Product image storage disk is not configured correctly.'],
            ]);
        }
    }

    private function buildProductImageFilename($image): string
    {
        ...
        if (! in_array($extension, ['jpg', 'png', 'gif', 'webp', 'bmp'], true)) {
            $extension = 'jpg';
        }

        return rand(11111111, 99999999).'_'.($safeName !== '' ? $safeName : 'product_image').'.'.$extension;
    }

    private function productImageLegacyPaths(string $filename): array
    {
        return [
    ...
    private function productImageExistsOnDisk(?string $filename): bool
    {
        ...
        try {
            return Storage::disk($this->productImagesDisk())->exists($this->productImageObjectKey($filename));
        } catch (\Throwable $e) {
            return false;
        }
    }

    private function existingProductImageSource(?string $filename): ?array
    {
        if (! $filename || $filename === 'no-image.png') {
            return null;
        }
        ...
        try {
            if ($this->productImageExistsOnDisk($filename)) {
                return [
                    'type' => 'disk',
                    'key' => $this->productImageObjectKey($filename),
                ];
            }
        } catch (\Throwable $e) {
        }

        foreach ($this->productImageLegacyPaths($filename) as $path) {
            if (is_file($path)) {
                return [
                    'type' => 'local',
                    'path' => $path,
    ...
    {
        $this->ensureProductImagesDiskAvailable();
        $filename = $this->buildProductImageFilename($image);
        $extension = strtolower(pathinfo($filename, PATHINFO_EXTENSION) ?: 'jpg');

        try {
            $imageResize = Image::make($image->getRealPath());
            $encoded = (string) $imageResize->resize(800, 800, function ($constraint) {
                $constraint->aspectRatio();
                $constraint->upsize();
            })->encode($extension);

            $saved = Storage::disk($this->productImagesDisk())->put(
                $this->productImageObjectKey($filename),
                $encoded,
                [
                    'visibility' => 'public',
                    'ContentType' => $image->getMimeType() ?: 'image/'.$extension,
                ]
            );

            if (! $saved) {
                throw new \RuntimeException('Cloud storage did not accept the image upload.');
            }
        } catch (\Throwable $e) {
            throw ValidationException::withMessages([
    ...
    private function duplicateProductImage(?string $filename): string
    {
        $source = $this->existingProductImageSource($filename);
        if (! $source) {
            return 'no-image.png';
        }
        ...
        $newImage = rand(11111111, 99999999).'_'.basename($filename);

        try {
            if (($source['type'] ?? null) === 'disk') {
                $copied = Storage::disk($this->productImagesDisk())->copy(
                    $source['key'],
                    $this->productImageObjectKey($newImage)
                );

                return $copied ? $newImage : 'no-image.png';
            }
            ...
            return $saved ? $newImage : 'no-image.png';
        } catch (\Throwable $e) {
            return 'no-image.png';
        }
    ...
    public function show_product_data($id, $variant_id, $warehouse_id = null)
    {
        ...
        $firstimage = explode(',', (string) $Product_data['image']);
        $item['image'] = $firstimage[0] ?? '';
        $item['image_url'] = product_image_url($item['image']);
    ...
    public function duplicate(Request $request, $id)
    {
        ...
        DB::transaction(function () use ($id, &$newProductId) {
            ...
            $newImage = $this->duplicateProductImage($original->image);
