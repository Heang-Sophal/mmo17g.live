...
class SettingsController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->hasFile('logo') && $request->file('logo')->isValid()) {
            ...
            $filename = rand(11111111, 99999999).basename((string) $image->getClientOriginalName());
            $encoded = (string) Image::make($image->getRealPath())
                ->resize(800, 800, function ($constraint) {
                    $constraint->aspectRatio();
                    $constraint->upsize();
                })
                ->encode(strtolower(pathinfo($filename, PATHINFO_EXTENSION) ?: 'jpg'));

            if (! media_put('app', $filename, $encoded, $image->getMimeType() ?: 'application/octet-stream')) {
                throw new \RuntimeException('Unable to save application logo to cloud storage.');
            }

            if ($setting->logo != 'logo-default.png') {
                media_delete('app', $currentAvatar);
            }
        } else {
        ...
        Setting::whereId($id)->update([
            ...
            'mobile_app_name' => $request['mobile_app_name'] ?? $setting->mobile_app_name,
    ...
    public function update_appearance_settings(Request $request, $id)
    {
        ...
        if ($request->hasFile('logo') && $request->file('logo') != $currentLogo) {
            ...
            $logoFilename = rand(11111111, 99999999).basename((string) $logo->getClientOriginalName());

            $saved = media_put(
                'app',
                $logoFilename,
                (string) Image::make($logo->getRealPath())->resize(80, 80)->encode(strtolower(pathinfo($logoFilename, PATHINFO_EXTENSION) ?: 'png')),
                $logo->getMimeType() ?: 'application/octet-stream'
            );
            if (! $saved) {
                throw new \RuntimeException('Unable to save appearance logo to cloud storage.');
            }

            if ($currentLogo && $currentLogo != 'logo-default.png') {
                media_delete('app', $currentLogo);
            }
        }
        ...
        if ($request->hasFile('favicon') && $request->file('favicon')->isValid()) {
            ...
            if (in_array($extension, ['ico', 'png'])) {
                ...
                $saved = media_put('app', $faviconFilename, file_get_contents($favicon->getRealPath()), $favicon->getMimeType() ?: 'image/x-icon');
                if (! $saved) {
                    throw new \RuntimeException('Unable to save favicon to cloud storage.');
                }

                // Delete old favicon if it exists and is not default
                if ($currentFavicon && $currentFavicon !== 'favicon.ico') {
                    media_delete('app', $currentFavicon);
                }
            }
        }
