...
class SettingsApiController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        if ($request->hasFile('logo')) {
            media_delete('store', $s->logo_path);

            $ext = strtolower($request->file('logo')->guessExtension() ?: 'png');
            $filename = (string) Str::uuid().'.'.$ext;

            $encoded = (string) Image::make($request->file('logo')->getRealPath())
        ...
        if ($request->hasFile('favicon')) {
            media_delete('store', $s->favicon_path);
            ...
            if ($ext === 'ico' || in_array($mime, ['image/x-icon', 'image/vnd.microsoft.icon'], true)) {
                ...
                $saved = media_put('store', $filename, file_get_contents($file->getRealPath()), $mime ?: 'image/x-icon');
            } else {
            ...
            if (! $saved) {
                throw new \RuntimeException('Unable to save store favicon to cloud storage.');
            }
        ...
        if ($request->hasFile('hero_image')) {
            media_delete('store', $s->hero_image_path);

            $ext = strtolower($request->file('hero_image')->guessExtension() ?: 'jpg');
            $filename = (string) Str::uuid().'.'.$ext;

            $encoded = (string) Image::make($request->file('hero_image')->getRealPath())
