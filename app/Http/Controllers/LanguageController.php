...
class LanguageController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('flag')) {
            ...
            $this->storeFlagImage($image, $filename);
    ...
    public function update(Request $request, Language $language)
    {
        ...
        if ($request->hasFile('flag')) {
            ...
            if ($language->flag && $language->flag !== 'no-image.png') {
                media_delete('flag', $language->flag);
            }
    ...
    private function storeFlagImage($image, string $filename): void
    {
        $extension = strtolower(pathinfo($filename, PATHINFO_EXTENSION) ?: 'png');

        if ($extension === 'svg') {
            $saved = media_put('flag', $filename, file_get_contents($image->getRealPath()), $image->getMimeType() ?: 'image/svg+xml');
        } else {
            $encoded = (string) \Image::make($image->getRealPath())->encode($extension);
            $saved = media_put('flag', $filename, $encoded, $image->getMimeType() ?: 'image/'.$extension);
        }

        if (! $saved) {
            throw new \RuntimeException('Unable to save language flag to cloud storage.');
        }
    }
}
