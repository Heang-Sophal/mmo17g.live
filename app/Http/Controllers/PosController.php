...
class PosController extends BaseController
{
    ...
    public function CreatePOS(Request $request)
    {
        ...
        try {
            $sale = \DB::transaction(function () use ($request, $totalPaid, $saleUuid) {
                ...
                $order->payment_method = $this->resolvePaymentMethodSummary($request->payments);
    ...
    private function sendTelegramNotification($sale)
    {
        ...
        $saleData = [
            ...
            'created_by' => $sale->user ? $sale->user->username : 'Unknown',
            'seller_name' => $sellerName,
            'seller_phone' => $sale->user->phone ?? 'N/A',
            'customer_address' => $sale->client->adresse ?? 'N/A',
    ...
    public function submit_sale_from_draft(Request $request)
    {
        ...
        if ($draft) {
            ...
            $sale = \DB::transaction(function () use ($request, $draft) {
                ...
                $order->statut = 'completed';
                $order->payment_statut = 'unpaid';
                $order->payment_method = $this->resolvePaymentMethodName(data_get($request->payment, 'payment_method_id'));
                $order->user_id = Auth::user()->id;
                    ...
                    if ($request['amount'] > 0) {
                        if ($request->payment['payment_method_id'] == 1 || $request->payment['payment_method_id'] == '1') {
                            ...
                            $sale->update([
                                ...
                                'payment_method' => $this->resolvePaymentMethodName($request->payment['payment_method_id']) ?: $sale->payment_method,
                            ]);
    ...
    private function resolvePaymentMethodSummary($payments): ?string
    {
        ...
        $namesById = PaymentMethod::whereIn('id', $methodIds->unique()->all())
            ->pluck('name', 'id');

        $methodNames = $methodIds
            ->map(fn ($id) => $namesById->get($id))
            ->filter()
            ->unique()
            ->values();

        return $methodNames->isEmpty() ? null : $methodNames->implode(', ');
    }

    private function resolvePaymentMethodName($paymentMethodId): ?string
    {
